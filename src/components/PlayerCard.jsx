    ...
    >
      <div className="flex items-start gap-3">
        <div className="relative flex-shrink-0">
          <div className="w-[68px] h-[68px] overflow-hidden" style={{ background: '#EEF2F7', border: '1px solid rgba(220,214,204,0.96)' }}>
            {player.photo_url ? (
              ...
              <div className="w-[68px] h-[68px] flex items-center justify-center">
                <span className="text-2xl font-black" style={{ color: '#0B1528', letterSpacing: '-0.05em' }}>{getInitials(fullName)}</span>
              </div>
            )}
          </div>
          {player.is_verified && (
            <div className="absolute -bottom-1 -right-1 w-6 h-6 flex items-center justify-center ring-2 ring-[#FFFFFF]" style={{ backgroundColor: '#C1121F' }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <h3 className="text-[20px] leading-[1.05] truncate" style={{ color: '#0B1528', fontFamily: 'ui-serif, Georgia, Cambria, Times New Roman, Times, serif', fontWeight: 500 }}>{fullName}</h3>
              <p className="text-xs font-semibold mt-1.5" style={{ color: '#5B6475' }}>
          ...
          <div className="flex items-center gap-3 mt-3">
            {player.city && player.state && (
              <div className="flex items-center gap-1 px-2.5 py-1" style={{ backgroundColor: '#F5F7FB' }}>
                <MapPin size={11} color="#8B95A7" />
                <span className="text-xs font-semibold" style={{ color: '#5B6475' }}>{player.city}, {player.state}</span>
              </div>
            )}
