...
async function upsertTournament(base44, source, record, dryRun) {
  ...
  try {
    ...
    for (const [index, record] of parsed.entries()) {
      try {
        ...
        const result = await upsertTournament(base44, source, record, dryRun);
...
export default async function(req) {
  try {
    ...
    for (const source of candidateSources) {
      const result = await runSource(base44, source, { stateFilter, sportFilter, maxEvents, dryRun });
      results.push(result);
